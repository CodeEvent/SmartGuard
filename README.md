...
powershell
.\demo.ps1
The demo runs four stages against stratos_unlimited.sol — a Solidity reconstruction of the Stratos Chain contract that received a HIGH-risk finding from BlockSec's professional audit:
...
🔧 Troubleshooting
<details> <summary><strong>Detectors not registering</strong></summary>
powershell
pip uninstall smartguard -y
pip install -e .
pip show smartguard
slither --list-detectors | Select-String "unlimited-mint"
</details> <details> <summary><strong>OZ contracts fail to compile (missing imports)</strong></summary>
Some OZ contracts import siblings that Slither can't resolve when run individually. This is expected — it does not affect the 0 findings true negative result since Slither still analyses what it can compile. Context.sol and ReentrancyGuard.sol compile cleanly in all cases.

</details> <details> <summary><strong>Wrong solc version</strong></summary>
powershell
solc-select use 0.8.0
solc --version    # must show 0.8.0+commit...
</details> <details> <summary><strong>Full dependency check</strong></summary>
powershell
...
🏗 Built On
Slither by Trail of Bits — the static analysis foundation
OpenZeppelin Contracts v5.2.0 — legitimate contract evaluation set
solc-select — compiler version pinning
<div align="center">
